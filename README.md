...
`개발일지` 메뉴에서 피드 글을 근거로 선택한 뒤 `개발일지 작성`을 누르면 사용자가 입력한 네이버 글 근거, GitHub 기록, 실제 개발 환경/작업감 메모를 바탕으로 개발일지 초안을 만들 수 있습니다. 형식은 `공부형`과 `당일 개발 경험` 중 선택하며, 공부형은 개념/배운 점/새롭게 느낀 점을 중심으로 구성합니다. 발행 시 피드 글처럼 `전체 공개` / `친구만` / `나만` 공개 범위를 직접 선택합니다. 초안 생성기는 입력된 근거가 없는 성과나 작업을 단정하지 않도록 "과장 방지 체크" 섹션을 함께 생성합니다.
